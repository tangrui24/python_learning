...
def start_session(argvs):
    ...
    if user:
        ...
        while not exit_flag:
            if user.bind_hosts:
                print('\033[32;1mz.\t ungrounded hosts (%s)\033[0m' % len(user.bind_hosts))
            ...
            if choice == 'z':
                print("------ Group: ungrounded hosts ------")
                for index, bind_host in enumerate(user.bind_hosts):
                    print("  %s.\t%s@%s(%s)" % (index, bind_host.remoteuser.username, bind_host.host.hostname,
                ...
                if choice < len(user.groups):
                    ...
                    for index, bind_host in enumerate(user.groups[choice].bind_hosts):
                        print("  %s.\t%s@%s(%s)" % (index, bind_host.remoteuser.username,
                    ...
                    while not exit_flag:
                        ...
                        if user_option.isdigit():
                            ...
                            if user_option < len(user.groups[choice].bind_hosts):
                                ...
                                #print('audit var:', user.groups[choice].bind_hosts[user_option].audit_logs)
